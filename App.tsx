...
const App: React.FC = () => {
  ...
  const [sidebarMode, setSidebarMode] = useState<'events' | 'todos'>('events');
                ...
                <div className="px-3 py-2 border-b border-gray-100 dark:border-gray-800/50 flex items-center justify-between shrink-0">
                   <div className="flex gap-2">
                      <button 
                        ...
                        className={`text-xs font-bold uppercase tracking-wider transition-colors ${sidebarMode === 'events' ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300'}`}
                      >
                         {t.todaySchedule}
                      </button>
                      ...
                      <button 
                        ...
                        className={`text-xs font-bold uppercase tracking-wider transition-colors ${sidebarMode === 'todos' ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300'}`}
                      >
                         {t.todoList}
                      </button>
                   </div>
                           ...
                           allTodosList.map(todo => (
                               <div key={todo.id} className="group flex items-center gap-2 p-2 rounded-lg hover:bg-white dark:hover:bg-gray-800 hover:shadow-sm border border-transparent hover:border-gray-100 dark:hover:border-gray-700 transition-all">
                                   <button 
                                      ...
                                      className={`shrink-0 ${todo.completed ? 'text-primary-500' : 'text-gray-300 dark:text-gray-600 hover:text-primary-500'}`}
                                   >
                                      {todo.completed ? <CheckSquare size={14} /> : <Square size={14} />}
                                   </button>
                                   <div className="flex-1 min-w-0">
                                       <div className={`text-xs truncate ${todo.completed ? 'line-through text-gray-400' : 'text-gray-700 dark:text-gray-300'}`}>
                                           {todo.text}
                                       </div>
                                       <div className="text-[9px] text-gray-400 flex gap-1">{todo.dateKey}</div>
                                   </div>
                               </div>
                           ))
                       )
